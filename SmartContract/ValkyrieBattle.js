...
ValkyrieBattleContract.prototype = {
        ...
        if(currentTime > (game.lastMoveTimestamp + ACTION_EXPIRE_TIMEOUT)){
            ...
            if(ownGameHash === game.playerGameHash[1 - game.currentPlayer]){
                ...
                if(ownGameLayout !== game.playerLayout[1 - game.currentPlayer]){
                    var creatorGameLayout = (game.currentPlayer % 2 === 0) ? game.playerLayout[0] : ownGameLayout;
                    var challengerGameLayout = (game.currentPlayer % 2 === 1) ? game.playerLayout[1] : ownGameLayout;
                    var result = this._verifyGameLayoutAndGetWinner(game.gameId, JSON.parse(creatorGameLayout), JSON.parse(challengerGameLayout));
                ...
                this._updateGameResult(gameId, winner, winningReason);
            }else{
                // Cheater
    ...
    _verifyGameLayoutAndGetWinner:function(gameId, creatorGameLayout, challengerGameLayout){
        ...
        var games = [creatorGame, challengerGame];
        for(var i = 0; i < game.attacks.length; i++){
            var step = game.attacks[i];
            if(i === (game.attacks.length - 1) && (step.state === -1)){
                continue;
            }

            if(games[step.player].shootAt({x: step.x, y: step.y}) !== step.state){
                result.winner = 1 - step.player;
                result.winner = 2;
            }
        }
        
        // No cheating found
        result.winner = 1 - game.currentPlayer;
