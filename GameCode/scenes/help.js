var SceneHelp = new Phaser.Class({
    ...
    create: function ()
    {
        ...
        var helpText = [
            ...
            '  3.游戏列表中也会列出自己创建的游戏，但无法挑战，只能挑战其他玩家的游戏',
            '  4.被挑战方会看到挑战申请，接受挑战后即可开始游戏，双方轮流点击对方棋盘进行攻击',
            '  5.轮到对方走时对方棋盘变灰不可用，轮到己方走时，对方棋盘变为可用状态，点击棋盘的空位进行攻击，',
            '    对方更新状态后会显示在棋盘上，X：击中飞机头，x：击中飞机，O：未击中飞机',
            '  6.当轮到己方走时，如有确定赢的把握，可申请结束游戏，在敌方棋盘上拖动飞机，确定敌方飞机位置后即可提交',
            '  7.对方收到终局申请后，如果位置无误则自动投降，如果位置错误，则提交飞机布局，由星云链的智能合约裁判',
            '  8.星云链的智能合约会确认双方飞机布局数据及每一步的数据是否准确，并根据最终结果判胜负',
            '  9.如有一方作弊提交错误数据，星云链的智能合约会检测出并自动判作弊方负',
            '  10.游戏开始后玩家必须在2分钟内走每一步，如果一方超时，另一方自动提出超时申请，区块链自动判超时方负',
            '',
            '注意事项：',
            '  *. 同一地址的玩家无法挑战自己创建的游戏',
